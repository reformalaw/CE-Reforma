	<h2>Quick contact</h2>
		<div class="quick-form">
			<?php if($sf_user->hasFlash('succMsg')) { ?>
				<p align="left" style="color:green;float:left; margin: -12px 0 0 7px; position: absolute;"><?php echo $sf_user->getFlash('succMsg');?></p>
			<?php }?>

			<form action="<?php echo url_for('home/index') ?>" method="post" name="casecontact" onsubmit="return validateForm()">
				<ul>
					<li>
						<?php echo $contactForm['name']->render(array( 'onblur' => "if(this.value=='') this.value=this.defaultValue;", 'onfocus' =>"if(this.value==this.defaultValue) this.value='';", 'value'=> "Name"));?>
					</li>

					<li>
						<?php echo $contactForm['email']->render(array( 'onblur' => "if(this.value=='') this.value=this.defaultValue;", 'onfocus' =>"if(this.value==this.defaultValue) this.value='';", 'value'=> "Email"));?>
					</li>
					
					<li>
						<?php echo $contactForm['phone']->render(array( 'onblur' => "if(this.value=='') this.value=this.defaultValue;", 'onfocus' =>"if(this.value==this.defaultValue) this.value='';", 'value'=> "Phone"));?>
					</li>

					<li>
						<?php echo $contactForm['message']->render( array('onfocus' => "if(this.value==this.defaultValue) this.value='';", 'onblur' => "if(this.value=='') this.value=this.defaultValue;", 'value' => 'Brief Message'));?>
					</li>
					
					<li>
						<input class="send-btn" name="submit" type="submit" value="Send" />
					</li>
			</form>
		</div>

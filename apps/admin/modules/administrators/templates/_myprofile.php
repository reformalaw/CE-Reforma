...
<tr valign="top">
 <td colspan="2" class="ListAreaPad">
  <form action="<?php echo url_for('administrators/myprofile') ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?> id="myProfileFrm" name="myProfileFrm">
...
*/ ?>
    ...
    <tr>
      <!--<td align="center" class="fldrowbg error">*</td>-->
      <td width="26%" class="fldrowbg"><b><?php echo $form['MiddleName']->renderLabel() ?>: </b></td>
      <td width="68%" class="fldrowlightbg">
    ...
    <tr>
      ...
      <td width="68%" class="fldrowlightbg">
		<?php $imagePath = clsCommon::userProfileImage($snId, "medium"); ?>
		<div id="defaultImage"> 
			<?php echo image_tag($imagePath['path'],array('border'=>'0','alt'=>'Image','title'=>$imagePath['title'],'width'=>'75px','height'=>'75px')); ?>
			<span><?php if(!array_key_exists("noImage",$imagePath)){ ?> <a title="Click to reset default image" style="font-size: 14px;margin: 0 0 0 40px;" href="javascript:void(0)" onClick='ajaxDefaultProfileImg("<?php echo $snId;?>");'>Set Default Image</a> <?php  } ?></span>
		</div>
		
      </td>
      
    </tr>
<!--//echo link_to("Default Photo",'administrators/ajaxDefaultImage?id='.$snId);-->
     <tr class="fldbg">
...
jQuery().ready(function() {
	...
	jQuery("#myProfileFrm").validate({
		...
		rules: {
			...
			"myprofile[New_Password]":{
				required: function(){
				    if( jQuery.trim(jQuery("#myprofile_Password").val()) != ""){
				        return true;
				    }else{
				        return false;
				    }
		    ...
		    "myprofile[Confirm_Password]":{
				required: false,
				equalTo: "#myprofile_New_Password"
		    },
		},
		messages: {
		    "myprofile[FirstName]": {
		    ...
		    "myprofile[MiddleName]": {
		      ...
		      minlength:  "Middle name  must be at least 1 characters long.",
		      maxlength:  "Middle name cannot be longer than 45 characters."
		    },
		    "myprofile[LastName]": {
		      required:   "This field is required.",
...
<script type="text/javascript">
function ajaxDefaultProfileImg(id)
{
	$.ajax( {
            type: "POST",
            url: "<?php echo url_for('administrators/ajaxDefaultImage'); ?>",
            data : { id : id},
            success: function(output){
				html = '<img width="75px" height="75px" border="0" src="/images/user-noImage/medium_noImage.png" title="no-image" alt="Image">';

	...
	<?php 
		...
		if ($pager->getPage() != $pager->getFirstPage())
		{
			...
			if($pager->getPreviousPage()==1)
			{
				...
				echo("<li><a href='".url_for($strUrl.$varExtraTmp)."' title='Previous'>".image_tag('legalgrip/page-left-arrow.png')."</a></li>");			
				
			}
			else {
				//echo("<li  class='previous'><a href='".url_for($strUrl.'?page='.$pager->getPreviousPage().$varExtra)."' title='Previous'>&#171; Previous</a></li>");			
				echo("<li><a href='".url_for($strUrl.'?page='.$pager->getPreviousPage().$varExtra)."' title='Previous'>".image_tag('legalgrip/page-left-arrow.png')."</a></li>");			
		...
		else
		{
			...
			echo "<li><a href='javascript:void(0);'>".image_tag('legalgrip/page-left-arrow.png')."</a></li>";
		...
		foreach ($links as $page)			
		{
			if($page==1)
			{
				if($varExtra!="")
					 $varExtraTmp = preg_replace("/&/","?",$varExtra,1);						
				echo(($page == $pager->getPage()) ? "<li class='active'><span>$page</span></li>" : "<li><a href='".url_for($strUrl.$varExtraTmp)."' title='".$page."'>".$page."</a></li>");					
			}
			else
			{				  
				echo(($page == $pager->getPage()) ? "<li class='active'><span>$page</span></li>" : "<li><a href='".url_for($strUrl.'?page='.$page.$varExtra)."' title='".$page."'>".$page."</a></li>"); 
			}
		}
		if ($pager->getPage() != $pager->getLastPage())
		{
			//echo("<li class='next'><a href='".url_for($strUrl.'?page='.$pager->getNextPage().$varExtra)."', title='Next'>Next &#187;</a></li>");
			echo("<li><a href='".url_for($strUrl.'?page='.$pager->getNextPage().$varExtra)."', title='Next'>".image_tag('legalgrip/page-right-arrow.png')."</a></li>");
		...
		{
			echo "<li><a href='javascript:void(0);'>".image_tag('legalgrip/page-right-arrow.png')."</a></li>";
